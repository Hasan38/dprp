<?php 
$id=mysqli_real_escape_string($conn,$_GET['ald']);
	$detail=mysqli_query($conn,"SELECT * from album,users where id_album='".$id."'");
	$n   = mysqli_fetch_array($detail);
	$tgl = tgl_indo($n['tgl_posting']);
	$hits = $n[hits_album]+1;
	mysqli_query($conn,"UPDATE album SET hits_album='$hits' where id_album='".$id."'");
?>
	...
	<div class="post-entry" itemprop="articleBody">
		<?php
		  	echo "<div class='post post-style-1'>
				  ...
				  $g = mysqli_query($conn,"SELECT * FROM gallery WHERE id_album='".anti_injection($_GET['ald'])."' ORDER BY id_gallery DESC LIMIT $posisi,$batas");
				  ...
				  if ($ada > 0){
					...
					$jmldata     = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM gallery WHERE id_album='".anti_injection($_GET['ald'])."'"));
