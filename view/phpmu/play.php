<?php
  $detail=mysqli_query($conn,"SELECT * FROM video,users,playlist WHERE users.username=video.username 
					   AND video.id_playlist=playlist.id_playlist AND id_video='".anti_injection($_GET[id])."'");
  $d   = mysqli_fetch_array($detail);
  $tgl = tgl_indo($d['tanggal']);
  $lihat = $d['dilihat']+1;
	
  mysqli_query($conn,"UPDATE video SET dilihat=$d[dilihat]+1 WHERE id_video='".anti_injection($_GET[id])."'");
?>	
  ...
  <div class="article-content clearfix">
    <header>
      ...
      <div class="figure-container">
        <figure class="featured-post-figure" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
          ...
          <?php 
          if (trim($d[video])!=''){
            echo "<video width='100%' height='445' controls><source src='img_video/$d[video]' type='video/mp4'>
    ...
    <div class="post-entry" itemprop="articleBody">
      ...
      <nav class="post-navigation clearfix">
        <?php
        $id = $d['id_video'] - 1;
        $prev = mysqli_query($conn,"SELECT * FROM video where id_video='$id'");
        $p = mysqli_fetch_array($prev);
        $hitung = mysqli_num_rows($prev);
        ...
        <?php
        $idd = $d['id_video'] + 1;
        $prevd = mysqli_query($conn,"SELECT * FROM video where id_video='$idd'");
        $pd = mysqli_fetch_array($prevd);
        $hitungd = mysqli_num_rows($prevd);
