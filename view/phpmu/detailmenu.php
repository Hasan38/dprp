<?php
$detail=mysqli_query($conn,"SELECT * FROM halamanstatis where judul_seo='".anti_injection($_GET['halaman'])."'");
		...
		<article class="article-container clearfix" itemscope itemtype="http://schema.org/Article">
                    <div class="article-content clearfix">
						...
						<div class="post-entry" itemprop="articleBody">
                            <?php 
                                if (trim($d['gambar'])!=''){
                                    echo "<img class='pull-right img-thumbnail' style='width:100%; margin-left:8px' src='foto_statis/$d[gambar]'>";
                                }
                                echo "$d[isi_halaman]"; 
                            ?>
                        </div>
