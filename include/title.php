<?php
if (isset($_GET['judul'])){
   $sql = mysqli_query($conn,"select judul from berita where judul_seo='".anti_injection($_GET[judul])."'");
	...
	$sql = mysqli_query($conn,"select jdl_video from video where id_video='".anti_injection($_GET[id])."'");
    $j   = mysqli_fetch_array($sql);
    if ($j) {
...
elseif ($_GET['ald'] != ''){
	$sql = mysqli_query("select jdl_album from album where id_album='".anti_injection($_GET[ald])."'");
    $j   = mysqli_fetch_array($sql);
    if ($j) {
...
elseif ($_GET['kt'] != ''){
	$sql = mysqli_query($conn,"select nama_kategori from kategori where id_kategori='".anti_injection($_GET[kt])."'");
    $j   = mysqli_fetch_array($sql);
    if ($j) {
...
$sql = mysqli_query($conn,"select tema from agenda where tema_seo='".anti_injection($_GET[tema])."'");
    $j   = mysqli_fetch_array($sql);
    if ($j) {
        echo "$j[tema]";
	}
...
elseif ($_GET['halaman'] != ''){
	$sql = mysqli_query($conn,"SELECT * FROM halamanstatis where judul_seo='".anti_injection($_GET[halaman])."'");
    $j   = mysqli_fetch_array($sql);
    if ($j) {
