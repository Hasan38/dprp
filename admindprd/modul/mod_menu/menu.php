...
session_start();
 ...
 echo "
  ...
  default:
   ...
   echo "<tr style='color:$color' class=gradeX> 
   <td>$r[nama_menu] $identitas</td>";
            
  $parent=mysql_query("SELECT * FROM menu WHERE id_menu=$r[id_parent] order by urutan");
  ...
  <td><center>$r[position]</center></td>
  <td><center>No $r[urutan]</center></td>
  ...
  echo "
   ...
   <label for=field4>Link berita</label>
   <input type=text style='width:40%' name='link' value=''> <i>Jika Tidak memakai Link Berita, maka kosongkan saja.</i>
  </p>   
   ...
   $tampil=mysql_query("SELECT * FROM menu WHERE id_parent='0' ORDER BY id_menu");
   while($r=mysql_fetch_array($tampil)){
   ...
   <label for=field4>Nama Menu</label>
    <input type=text style='width:40%' name='nama_menu'>
   </p> 
   	
   <p class=inline-small-label> 
   <label for=field4>Positon</label>
   ...
   <label for=field4>Urutan</label>
    <input type=text style='width:10%' name='urutan'>
   </p> 
  ...
  <input type='submit' name='upload' class='button' value=' Simpan &nbsp;&nbsp;&nbsp;&nbsp;'>
   ...
   <p class=inline-small-label> 
   <label for=field4>Positon</label>
   <input type=radio name='position' value='Top' checked>Top 
   <input type=radio name='position' value='Bottom'>Bottom
   </p>";}
   ...
   <label for=field4>Positon</label>
   <input type=radio name='position' value='Top'>Top 
   <input type=radio name='position' value='Bottom' checked>Bottom
   </p>";}

   echo"
   <p class=inline-small-label> 
   <label for=field4>Urutan</label>
    <input type=text style='width:10%' name='urutan' value='$r[urutan]'>
   </p>
   <p class=inline-small-label> 
