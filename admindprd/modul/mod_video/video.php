...
switch($_GET[act]){
  ...
  default:
   ...
   <a href='$aksi?module=video&act=hapus&id=$r[id_video]&namafile=$r[gbr_video]&namafile2=$r[video]'  onclick=\"return confirm('Apa anda yakin untuk hapus Data ini?')\" 
  ...
  <input type=text name='jdl_video' size=60>
   ...
   <textarea id='loko' name='keterangan' style='width: 720px; height: 250px;'></textarea>
   </p> 	
   ...
   <span class style=\"color:#EA1C1C;\">Tipe video harus MP4
   </p> ";
  ...
  break;
   ...
   <p class=inline-small-label> 
   <label for=field4>Judul Video</label>
  <input type=text name='jdl_video' size=60 value='".cetak($r[jdl_video])."'>
   </p> 		  
   ...
   $tampil=mysql_query("SELECT * FROM playlist ORDER BY jdl_playlist");
   if ($r[id_playlist]==0){
   echo "<option value='' selected>- Pilih Playlist -</option>"; }   
   ...
   <p class=inline-small-label> 
   <label for=field4>Keterangan</label>
   <textarea id='loko' name='keterangan' style='width:720px; height: 250px;'>$r[keterangan]</textarea>
   </p> 	 
   ...
   Youtube <br><iframe width='250' height='170' src='$r[youtube]' frameborder='0' allowfullscreen></iframe></p>";
    }
   ...
   <span class style=\"color:#EA1C1C;\">Tipe video harus MP4
    ...
    if (trim($r[video])!=''){
       echo "<p class=inline-small-label> 
   <label for=field4>Video Upload : </label>Video Upload <br>
       <video width='250' height='170' controls>
              <source src='../img_video/$r[video]' type='video/mp4'>
            Your browser does not support the video tag.
            </video>";
            
            echo "<br/><br/><a href='../play-$r[id_video]-$r[video_seo].html' target='blank' class='button red'> Test Play on website </a>
   </p>";
    }
